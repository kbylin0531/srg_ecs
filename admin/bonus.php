...
//-- 处理红包的发送页面
    ...
    {
        global $db, $ecs, $smarty, $_LANG, $_CFG;
        $user_list = array();
...
//-- 添加发放红包的商品
    /*------------------------------------------------------ */
    public function add_bonus_goods()
    {
        global $db, $ecs;
        include_once(ROOT_PATH . 'includes/cls_json.php');
        $json = new JSON;
...
//-- 删除红包
    ...
    private function send_bonus_mail($bonus_type_id, $bonus_id_list)
    {
        ...
        foreach ($bonus_list AS $bonus) {
            ...
            if ($this->add_to_maillist($bonus['user_name'], $bonus['email'], $tpl['template_subject'], $content, $tpl['is_html'])) {
