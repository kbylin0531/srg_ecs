...
require __DIR__ . '/../sharin7/Sharin/web.inc';

spl_autoload_register(function ($clsnm) {
    $file = SR_PATH_BASE . 'includes/' . $clsnm . '.php';
...
@ini_set('include_path', (DIRECTORY_SEPARATOR == '\\' ? '.;' : '.:') . ROOT_PATH);
...
/* 对用户传入的变量进行转义操作。  PHP5.4以后get_magic_quotes_gpc()始终返回false */
empty($_GET) or $_GET = addslashes_deep($_GET);
empty($_POST) or $_POST = addslashes_deep($_POST);
empty($_COOKIE) or $_COOKIE = addslashes_deep($_COOKIE);
empty($_REQUEST) or $_REQUEST = addslashes_deep($_REQUEST);
