...
class License
{
    ...
    {
        $table = $GLOBALS['ecs']->table('shop_config');
        // 取出网店 license
        $sql = "SELECT code, value FROM {$table} WHERE code IN ('certificate_id', 'token', 'certi') LIMIT 0,3";
     ...
     * @param   string $token 证书token
     * @return  string|null
     */
    public static function make_shopex_ac($post_params, $token)
    {
     ...
     * @param   int $use_lib 使用哪一个json库，0为ec，1为shopex
     * @return  array
     */
    public static function exchange_shop_license($certi, $license, $use_lib = 0)
    {
        if (!is_array($certi)) {
            return array();
        }
     ...
     * @return  array
     */
    public static function process_login_license($cert_auth)
    {
     ...
     */
    public static function license_login($certi_added = [])
    {
        // 登录信息配置
        $certi = self::getLoginInfo(); // 网店证书ID
        // 标识
     ...
     *                    $return_array['request']；
     */
    public static function license_reg($certi_added = [])
    {
        // 登录信息配置
        $certi = self::getLoginInfo();
        // 标识
        $certi_back['succ'] = 'succ';
        $certi_back['fail'] = 'fail';
        // return 返回数组
...
{
    return License::get_shop_license();
}
...
{
    return License::make_shopex_ac($post_params, $token);
}
...
{
    return License::exchange_shop_license($certi, $license, $use_lib);
}
...
{
    return License::process_login_license($cert_auth);
}
...
{
    return License::license_login($certi_added);
}
...
{
    return License::license_reg($certi_added);
}